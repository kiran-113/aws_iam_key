...
EOF
 tags               = merge(local.tags, tomap({"Lambda" = local.lambda_find_users_to_refresh_name}))
}
...
EOF
tags               = merge(local.tags, tomap({"Lambda"=local.lambda_update_iam_credentials_for_user_name}))
}
